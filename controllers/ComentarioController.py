...
def get_comentarios(db, search, page, per_page):
    try:
        offset = (page - 1) * per_page

        sql_query = text(
            """
            SELECT COUNT(*) AS Filas FROM comentario
            WHERE
            (
                LOWER(CONCAT(CAST(comentario_id AS CHAR), comentario_nombre, comentario_nombre))  LIKE LOWER( CONCAT( '%',:search,'%'))
            ) AND status=1
            """
        )

        comentarios = db.execute(sql_query, {"search": search}).fetchone()

        row = comentarios.Filas
        ...
        sql_query = text(
            ...
            WHERE
            (
                LOWER(CONCAT(CAST(c.comentario_id AS CHAR), c.comentario_nombre, c.comentario_descripcion)) LIKE LOWER( CONCAT( '%',:search,'%'))
            ) AND c.status=1
            {limit_query}
            """
        )
        ...
        comentarios = [
            {
                ...
                "comentario_nombre": comentario.comentario_nombre,
                "comentario_descripcion": comentario.comentario_descripcion,
            }
...
def get_comentario(db, comentario_id):
    try:
        sql_query = text(
            """
        ...
        comentario = db.execute(sql_query, {"comentario_id": comentario_id}).fetchone()
